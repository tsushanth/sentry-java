...
public class SentryNavStateHolder<T : Any>(
  ...
  public fun onBackstackChanged(backStack: List<T>) {
    currentBackStack = backStack

    if (backStack.isEmpty()) {
      if (enableBackstackContext) {
        scopes.configureScope { scope -> scope.removeContexts("navigation") }
      }
  ...
  public fun onEntryVisible(contentKey: Any, metadata: Map<String, Any>) {
    val key = resolveKey(contentKey) ?: return
    visiblePanes[contentKey] = VisiblePane(key = key, contentKey = contentKey, metadata = metadata)
    updateNavigationContext()
    applyPrimaryChange(selectPrimaryPane(visiblePanes.values)?.key)
  }

  public fun onEntryHidden(contentKey: Any) {
  ...
  public fun onEntryPopped(contentKey: Any) {
    visiblePanes.remove(contentKey)
    updateNavigationContext()
  ...
  private fun updateNavigationContext() {
    ...
    if (context.isEmpty()) {
      ...
      scopes.configureScope { scope -> scope.setContexts("navigation", context) }
    }
  }
  ...
  private fun contentKeyMatches(key: T, contentKey: Any): Boolean {
    ...
    return key.toString() == contentKey.toString()
  }
  ...
  private fun resolveArguments(key: T): Map<String, Any?> {
    val raw = argumentsExtractor?.invoke(key) ?: return emptyMap()
    return sanitizeArguments(raw)
  }
